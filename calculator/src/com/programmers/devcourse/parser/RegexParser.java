...
public class RegexParser implements Parser {
  ...
  private final Pattern pattern = Pattern.compile("([+*-/])|([0-9]{1,7}(\\.[0-9]{1,7})?)");

  private final Pattern operatorPattern = Pattern.compile("[+*-/]");
  private final Pattern numberPattern = Pattern.compile("[0-9]{1,7}(\\.[0-9]{1,7})?");
  private final Pattern acceptablePattern = Pattern.compile(
      "^(([+*-/])|([0-9]{1,7}(\\.[0-9]{1,7})?)|\\s)+$");
