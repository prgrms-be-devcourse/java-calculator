...
public class CalculateTest {
    ...
    @ParameterizedTest
    @CsvSource(value = {"7 + 3 : 10", "1 + 2 : 3", "5 + 7 + 17 : 29"}, delimiter = ':')
    @DisplayName("CalculatorService.calculate(String) - 덧셈 연산 테스트")
    void 덧셈(String input, String output) {
        assertEquals(calculatorService.calculate(input).getFormula(), calculatorManager.removeSpaces(input));
        assertEquals(calculatorService.calculate(input).getAnswer(), output);
    }
    ...
    @ParameterizedTest
    @CsvSource(value = {"4 * 5 / 4 + 7 * 2 : 19", "10 + 5 * 3 / 3 : 15", "15 * 2 - 17 : 13"}, delimiter = ':')
